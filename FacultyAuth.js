...
const FacultyAuth = ({ navigation }) => {
  ...
  const handleAuth = async () => {
    ...
    try {
      if (isLogin) {
        // Login existing faculty
        await signInWithEmailAndPassword(auth, email, password);
        navigation.navigate('FacultyDashboard');
      } else {
        // Register new faculty - first step
  ...
  const handlePersonalDetailsSubmit = async () => {
    if (!name || !facultyId || !department || !contactNumber) {
      Alert.alert('Error', 'Please fill all faculty details');
      return;
    }
    ...
    try {
      ...
      Alert.alert('Error', 'Failed to save faculty details');
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        {!showPersonalDetails ? (
          <>
            <Text style={styles.title}>{isLogin ? 'Faculty Login' : 'Faculty Registration'}</Text>
            ...
            {loading ? (
              ...
              >
                <Text style={styles.authButtonText}>
                  {isLogin ? 'Login' : 'Continue to Faculty Details'}
                </Text>
              </TouchableOpacity>
            )}
